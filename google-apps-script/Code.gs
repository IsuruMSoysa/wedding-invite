 ...
 * 4. Copy the Web App URL into .env as GOOGLE_SHEETS_SCRIPT_URL
 *
...
function doPost(e) {
  try {
    ...
    var name = (e.parameter && e.parameter.name) ? String(e.parameter.name).trim() : "";
    var attendance = (e.parameter && e.parameter.attendance) ? String(e.parameter.attendance).trim() : "";
    ...
    var attendanceLabel = attendance === "yes" ? "Joyfully Accept" : "Regretfully Decline";
...
function doGet() {
  return ContentService.createTextOutput(
    JSON.stringify({ ok: true, message: "RSVP endpoint is live. Use POST with name and attendance." })
